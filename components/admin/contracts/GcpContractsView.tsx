...
// --- PERBAIKAN TIPE PROPS DI SINI ---
interface GcpContractsViewProps {
  onEdit: (contract: Contract | GwContract) => void; // Terima tipe yang lebih umum
  onDelete: (id: string) => void;
  deletingContractId: string | null;
}

export function GcpContractsView({
  onEdit,
  onDelete,
  deletingContractId,
}: GcpContractsViewProps) {
  const { contracts, contractsPagination, fetchContracts, loading } =
    useAdminStore();
  ...
  useEffect(() => {
    ...
    fetchContracts(monthToSend, yearToSend, currentPage, ITEMS_PER_PAGE);
  }, [filterDate, currentPage, fetchContracts]);
  ...
  const totalPages = contractsPagination?.total_pages || 1;
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex items-center space-x-2 mb-4">
          ...
          >
            All ({contractsPagination?.total_items || 0})
          </Button>
          <Button
        ...
        <ContractsTable
          ...
          onDelete={onDelete}
        />
        {contractsPagination && contractsPagination.total_pages > 1 && (
